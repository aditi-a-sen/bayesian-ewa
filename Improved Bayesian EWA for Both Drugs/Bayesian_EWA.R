     ...
     for(i in 1:n1){
        ...
        for(t in 2:n3){
          ...
          for(k in 1:n2){
            ...
            for(g in 1:max(m)){
              p[1,i,j,k,g]<-p_temp[1,i,j,k,g]/sum(p_temp[1,i,j,k,1:m[i,j,k]])
              }
            ...
            for(t in 2:n3){
              ...
              for(g in 1:max(m)){
                 ...
                 V[t,i,j,k,g]<-((phi[i,j]^t)*V0[i,j,k,g]*N0 + inprod(f[1:t,i,j,k,g], (phi[i,j]^(t - c(1:t)))))/N[t,i,j]
                 f[t,i,j,k,g]<-(delta[i,j] + (1 - delta[i,j])*equals(y[t,i,j,k], g))*pi[i,j,k,t,g]
                 }
              for(g in 1:max(m)){
                 p[t,i,j,k,g]<-p_temp[t,i,j,k,g]/sum(p_temp[t,i,j,k,1:m[i,j,k]])
                 }
